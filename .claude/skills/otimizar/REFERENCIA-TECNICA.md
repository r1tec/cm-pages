# Referência técnica de preparação e desempenho

Consulta por assunto para páginas novas ou otimização solicitada. A divisão
aprovada pelo dono em 12/09/2026 substitui a regra anterior de executar este
roteiro em toda publicação. Leia somente as seções pertinentes; o escopo e a
profundidade são definidos em [SKILL.md](SKILL.md). Exemplos e métricas abaixo
são aprendizados técnicos, não autorização para novas ações ou gates universais.
Chat somente se solicitado. Publicação depende de autorização aplicável.
...
- `preparar.py` gera builds com manifesto usando `otimizar.py`;
  `estatico.py` pré-renderiza o ramo bundler.
...
- Construir localmente: `python3 preparar.py <slug> --saida /tmp/<build-da-tarefa>`.
...
- Em preparação ampla, usar `preparar.py --conferir` e conferir visual em celular/desktop.
  O verificador devolve falhas/avisos; publicar não executa essa auditoria por padrão.
  Corrigir defeitos introduzidos; documentar
...
- Única entrada: `./publicar.sh --build <build-validado> <slug>` (ou reuso padrão
  com `./publicar.sh <slug>`). Não usar FTP paralelo, workflow GitHub ou
...
- Em otimização ampla, consultar PageSpeed via `medir.py` e PSI_API_KEY do .env
  sem expor credenciais. Comparar a URL pública mobile **e** desktop após os
  ajustes relevantes publicados, reutilizando a referência válida. Registrar URL,
...
- Em otimização completa, seguir o escopo de [SKILL.md](SKILL.md):
  revisar todos os audits e executar melhorias aplicáveis mesmo
